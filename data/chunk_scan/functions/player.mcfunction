...
execute unless predicate chunk_scan:in_the_end if entity @e[distance=..256,type=area_effect_cloud,tag=chunk_scan.chunk] positioned ~-8 0 ~-8 as @e[distance=0..,type=area_effect_cloud,tag=chunk_scan.chunk,sort=nearest,limit=1] at @s if score $world.x scan > $min_x scan if score $world.x scan < $max_x scan if score $world.z scan > $min_z scan if score $world.z scan < $max_z scan run function chunk_scan:chunk/generate
